...
## Tab Navigation — Swipe Gestures (spec'd 2026-08-26, not yet built)

**What this is:** left/right swipe-to-change-tab between the 5 visible tab screens, in addition to the existing tap-to-switch behavior — a pure interaction/motion addition, no screen's content, data layer, or store changes. Today's `Tabs` (from `expo-router`, wrapping `@react-navigation/bottom-tabs`) does not support this at all; tabs only switch by tapping the bottom bar icon.

### Feasibility — investigated, not assumed

Confirmed by inspecting the actually-installed `apps/mobile/node_modules` (Expo SDK `~57.0.8`, `expo-router` resolved to `57.0.13`, `react-native` `0.86.0`, `react-native-reanimated` `4.5.0`, `react-native-safe-area-context` `~5.7.0`, `react-native-screens` `~4.26.0`):

- **`expo-router` 57 ships this exact pattern as a first-party primitive.** Its own source (`build/layouts/withLayoutContext.js`) documents, in a JSDoc example, precisely the pattern this feature calls for: `createMaterialTopTabNavigator` (`@react-navigation/material-top-tabs`) wrapped with `withLayoutContext` from `expo-router`. As of this version, expo-router pre-wires that exact combination and exposes it as `TopTabs`, importable from the `expo-router/js-top-tabs` entry point (`build/layouts/TopTabsClient.js`: `withLayoutContext(createMaterialTopTabNavigator().Navigator)`) — confirming the task's proposed approach is not just *a* valid approach, it's the one expo-router itself ships and documents for this SDK version. No alternative architecture (e.g., a hand-rolled `PagerView` + manual route sync) is needed or preferable.
- **`expo-router` vendors its own copy of `@react-navigation/material-top-tabs`'s navigator/view code** (`build/react-navigation/material-top-tabs/`) — so the external `@react-navigation/material-top-tabs` npm package does **not** need to be added as a separate dependency. Its actual runtime engine does, though: `MaterialTopTabView.js` does a runtime `require('react-native-tab-view')` and throws *"Install the 'react-native-tab-view' package and its peer dependencies to use the Expo Router's TopTabs"* if it's missing — confirming `react-native-tab-view` is the real new dependency this feature needs, not `@react-navigation/material-top-tabs` directly.
- **New dependencies required (none currently in `apps/mobile/package.json`):** `react-native-tab-view` (the pager/gesture engine `TopTabs` requires at runtime) plus its own peer dependencies `react-native-pager-view` and `react-native-gesture-handler` — neither installed today. `react-native-reanimated` (4.5.0) and `react-native-safe-area-context` (~5.7.0), already present, satisfy the rest of the chain. `code-generator` should add these via `npx expo install` (not raw `npm install`) so the versions resolved are the ones Expo SDK 57 actually blesses, per this repo's "check exact versioned behavior" convention (`apps/mobile/AGENTS.md`).
- **New Architecture compatibility — no blocking incompatibility found.** Expo SDK 57 / RN 0.86 is New-Architecture-only (Expo dropped Legacy Architecture support as of SDK 54; there is no opt-out). Expo Router shipping and documenting `TopTabs` as a supported feature *for this exact SDK* is itself strong evidence it works under New Architecture — a first-party Expo primitive would not ship broken against Expo's own now-mandatory-only architecture. `react-native-pager-view` and `react-native-gesture-handler` have both had Fabric/New-Architecture support for a long time. This remains a real (if low) risk area worth an explicit smoke test on a real device/simulator once built — per `harness/rules/ai-agents.md`'s real-environment testing discipline — not a reason to avoid the approach.
- **Library/version specifics belong in `spec/architecture.md`'s `## Stack`** (updated alongside this section) — this section documents the interaction design, not the dependency pins.

### The 5-screen swipe set

Swipe order matches the current tab bar's left-to-right order exactly: **Home → Portfolio → Health → Ask AI → More**. These are the only 5 participating screens.

**Explicitly excluded, and why:**
- **Alerts, Impulse Analyzer, Market News** (`alerts.tsx`, `impulse.tsx`, `news.tsx`) — today registered inside the same navigator with `href: null` so they're reachable only via a row on the More screen, never shown in the tab bar. They must **not** become swipeable: a swipe landing on a screen with no corresponding tab-bar icon would be disorienting and break the "the tab bar always reflects where you are" contract. **Structural requirement, not just a visual one:** these three must not be declared as children of the `TopTabs` navigator at all (unlike `href: null` under the current bottom-tabs `Tabs`, which only hides a route from the tab bar UI while still registering it in the navigator's route list) — under a swipeable pager navigator, any route registered in its `state.routes` is a route the pager can physically land on mid-swipe, regardless of tab-bar visibility. They must remain reachable only via `router.push` from More, structurally outside the 5-route pager (e.g., as sibling `Stack` routes within the `(tabs)` group rather than `TopTabs.Screen` children) — the exact file/route layout is `code-generator`'s implementation choice within this constraint.
- **Stock Detail** (`stock/[symbol].tsx`, pushed via `router.push` from Home) and the **`(auth)` group** (Login) — both live entirely outside the tab navigator today and stay that way; neither is reachable by a swipe gesture, and this feature does not touch either.

### Gesture-conflict review (all 5 participating screens read directly)

No genuine horizontal-gesture conflicts found on any of the 5 screens:

| Screen | Horizontal-gesture surfaces found | Verdict |
|---|---|---|
| Home (`index.tsx`) | None — one vertical `ScrollView`, a search `TextInput`, tap-only segmented control, tap-only result rows | Safe |
| Portfolio (`portfolio.tsx`) | None on the base screen (vertical `FlatList`/`ScrollView`, tap-only filter segments); 3 full-screen `<Modal>` overlays (Add/Sell/Edit) | Safe — React Native `Modal` renders in its own native layer above the navigator and captures all touches while open, independent of whatever navigator sits underneath |
| Health (`health.tsx`) | None — vertical `ScrollView`, a static SVG gauge with no gesture recognizer, a purely decorative flex-based horizontal diversification bar with zero touch handlers (not a scrollable) | Safe |
| Ask AI (`chat.tsx`) | None — vertical `FlatList` chat log, multiline `TextInput` composer, `KeyboardAvoidingView` | Safe |
| More (`more.tsx`) | None — vertical stack of tap-only rows, one tap-only segmented control | Safe |

The one general pattern present on every screen (a vertical `ScrollView`/`FlatList` nested inside what becomes a horizontal pager) is the standard non-conflicting case — perpendicular gesture axes resolve automatically via directional gesture-recognition (a vertical drag is claimed by the inner scroll view, a horizontal drag by the outer pager), the same behavior every native iOS/Android app with vertically-scrolling swipeable tabs already relies on. No carousel, drag-to-dismiss, swipeable-row-action, or slider pattern exists on any of the 5 screens today.

### Behavior-parity requirements (things the current bottom-tabs `Tabs` gives "for free" that a custom top-tabs-based bar must replicate deliberately)

1. **Keyboard handling.** Today's `screenOptions.tabBarHideOnKeyboard: true` hides the tab bar when the keyboard is open (relevant on Ask AI's composer). `@react-navigation/material-top-tabs` has no equivalent built-in option — the custom tab bar component must replicate this (hide/animate out on keyboard-open, restore on keyboard-close) so Ask AI's typing experience is unchanged.
2. **Scene background continuity.** Today's `screenOptions.sceneContainerStyle` keeps every tab's background at the canvas color during a tab switch. The top-tabs equivalent is a per-`TopTabs.Screen` `options.sceneStyle` — each of the 5 screens' `sceneStyle` must be set to `tokens.canvas` so a mid-drag frame never flashes a default/white background between screens.
3. **Bottom safe-area inset.** `@react-navigation/bottom-tabs` handles the tab bar's own bottom safe-area padding automatically; a fully custom bottom-bar component (restyled from `TopTabs`' default `MaterialTopTabBar`, which is a *top*-positioned bar by default) must handle this itself so the bar sits correctly above the home indicator/nav bar on both platforms, matching today's exact placement.
4. **Same 5 icons, same tint logic.** The custom tab bar must render the same `IconSymbol`/`SymbolView` icons, in the same order, with active/inactive tint colors sourced from `useThemeTokens()` exactly as `apps/mobile/app/(tabs)/_layout.tsx` does today — this is a visual restyle of the default `MaterialTopTabBar` into the current bottom-bar look, not a new visual design.

### Visual / motion requirement — "very smooth" per the user's request

The swipe must feel like a native iOS/Android swipeable-tabs interaction, not a canned transition:

- **Direct 1:1 finger-tracking pager transition** — the screen must track the user's drag distance in real time (the standard `react-native-tab-view`/`PagerView` drag behavior), not a fixed-duration fade/slide-in animation triggered only on gesture release.
- **The tab bar's active-icon indicator must update in sync with the drag**, not just snap at the end — as the user drags from tab N toward N+1, the active tint should visibly interpolate between the two icons' tint colors proportional to drag progress, matching the fractional-position feel of native tab-swipe UIs. (`react-native-tab-view`'s `TabView` exposes a fractional `position` value into the scene/tab-bar render path for exactly this purpose.)
- **No blank mid-swipe frames.** Whichever tab(s) are immediately adjacent to the currently focused tab must already be mounted (not lazily deferred) so a fast swipe never reveals an empty/loading placeholder mid-drag — lazy mounting, if used at all, should only defer tabs that are not adjacent to the current position.
- Respect the existing reduce-motion accessibility bar (`harness/patterns/ui-ux.md`) — if the OS-level reduce-motion setting is on, fall back to the same non-tracking, quicker transition every other animated element in this app already falls back to; this is not a new exception.

### Success Criteria
- [ ] Swiping left/right from any of the 5 tabs (Home, Portfolio, Health, Ask AI, More) navigates to the adjacent tab in the documented order, with the tab bar's selected icon updating to match.
- [ ] Swiping past the leftmost tab (Home) or rightmost tab (More) does nothing (no wrap-around, no landing on Alerts/Impulse/News/Stock Detail/Login).
- [ ] Alerts, Impulse Analyzer, and Market News remain reachable only via their More-screen rows — never reachable by a swipe gesture from any of the 5 tabs.
- [ ] Stock Detail and the `(auth)` group are structurally unaffected — pushing/popping Stock Detail from Home, and the entire pre-auth flow, behave identically to today.
- [ ] Portfolio's Add/Sell/Edit modals, Ask AI's composer + keyboard handling, and every existing per-screen interaction (search, filters, pull-to-refresh, Bottleneck Report handoff) work identically to today once the navigator change is built.
- [ ] The tab bar hides on keyboard-open on Ask AI (parity with today's `tabBarHideOnKeyboard`), and no screen shows a background-color flash during a tab transition.
- [ ] The swipe transition tracks the finger 1:1 during the drag (verified by scrubbing partway and releasing back to the origin tab — the screen should follow the finger, not just animate on release) and the tab bar's icon tint visibly interpolates mid-drag, not just at rest.

## Views / Screens (mobile — `apps/mobile/app`)

**Nav shell restructuring, started 2026-08-25, complete and shipped for all 9 screens:** the tab bar moved from 7 direct tabs to a **5-tab structure** — Home, Portfolio, Health, Ask AI, More — as part of the visual redesign above. Home, Stock Detail, and More shipped first (built and running); a second batch built the remaining 6 screens (Portfolio, Health, Ask AI, Alerts, Impulse Analyzer, Market News) against the same Design System tokens — **all built, qa-auditor-verified, and shipped as of 2026-08-25.** All 9 screens share one rule for this whole redesign: **presentation-only** — no screen in this initiative rewires its data layer; where the approved design calls for something with no real backing data (a chart, a metric, a detail view), that specific element is removed or simplified to what the app actually has, never stubbed as fabricated data or built as a "coming soon" placeholder. **Stock Detail's three "coming soon" stubs (price/chart, signal drivers, more-on-this-stock) were a deliberate, spec'd exception to this rule from 2026-08-25 through 2026-08-26** — see that screen's own entry for the reversal: the user rejected the stub treatment outright ("it is not coming soon... if there are things which we don't have, just don't include them"), and the exception no longer exists; Stock Detail now follows the same remove-don't-stub rule as every other screen.
...
### Screen: Stock Detail (`stock/[symbol].tsx`) — REDESIGNED 2026-08-25; stub sections removed 2026-08-26

**Purpose:** Give the user the full picture on one specific stock — its score and why it scored that way — reached from Home's ranked-row tap target or a search result. Same underlying feature (Feature 1, Stock Analytics Dashboard, `spec/roadmap.md` Build Status row 1) as Home; this is the detail view Home links to. See also `spec/capabilities/stock-analytics-dashboard.md`.

**Data source — real data, presentation-only redesign:** `GET /stocks/{symbol}/score` (`timeframe` param — `spec/api.md`), already wired via `getStockDetailScore` (`src/api/stockService.ts`). Response: `{symbol, timeframe, overall, technical, safety, sentiment, last_updated}`. No new endpoint, no new wiring.

**2026-08-26 reversal — stub sections removed, not stubbed:** the 2026-08-25 redesign spec'd this screen's price/chart, signal-drivers, and "more on this stock" sections as honest "coming soon" visual stubs — the one deliberate exception to the redesign's otherwise-universal "remove, don't stub" rule (see the nav-shell intro note above). The user explicitly rejected this treatment ("we don't have a chart, and it is not coming soon... if there are things which we don't have, just don't include them"). All three stub sections are removed from this spec entirely, below — no placeholder copy, no disabled controls, no "coming soon" text anywhere on this screen. The exception is retired; Stock Detail now follows the same rule as every other redesigned screen. The header row, timeframe control, score hero, and "Why this score?" pillar breakdown — all backed by real data — are unaffected. Full future-work reasoning for why these sections have no real data yet still lives in `spec/capabilities/stock-analytics-dashboard.md`'s "Known Gaps / Future Work."
...
**Removed from design (no fabricated data, no stub — 2026-08-26):** the design brief called for a price + chart section directly below the score hero (current price, daily change, % change; a line chart with 1D/1W/1M/3M/1Y segments, minimal gridlines, scrub-to-see-tooltip). No price-fetching code or endpoint exists anywhere in `apps/mobile` or `spec/api.md` today. This section — including any "coming soon" copy, disabled timeframe-segment control, or placeholder chart glyph — is removed entirely, not rendered in any form. The screen goes directly from the score hero (item 3 above) to "Why this score?" (item 4 below). See `spec/capabilities/stock-analytics-dashboard.md`'s "Known Gaps / Future Work" for what real price/chart support would require.

4. **"Why this score?" section.** Heading "Why this score?", 19px/650 weight. Three pillar rows — Technical, Safety, Sentiment, in that order — each: label (left) + numeric value (right-aligned, colored per the standing score bands) or "Not Available" text (existing behavior, restyled, not changed — per `spec/data.md`, `sentiment_score_*` is the only pillar field that can hold `"Not Available"`; `technical_score_*`/`safety_score_*` are always numeric) + a one-line explanatory note below the label + an optional thin progress bar (track `#1A1E1A`, filled lime `#C7FF3D`, present only when the pillar has a numeric value; absent — not zero-width — when "Not Available"). Progress bar width uses the same -100..100-to-0-100% normalization as Home's signal-strength bar: `(score + 100) / 2`.
...
**Removed from design (no fabricated data, no stub — 2026-08-26):** the design brief also called for a "Signal drivers" section (Momentum / Trend strength / Volume confirmation / Financial safety, each a tappable evidence row) and a "More on this stock" section (five expandable disclosure rows: Fundamentals, Earnings & financials, News & sentiment, Peer comparison, Score history). No per-driver breakdown is computed or stored anywhere in `spec/data.md`'s score tables, and none of the five disclosure rows have a backing data source today. Both sections — including any "Coming soon" row text, disabled chevrons, or expandable stub bodies — are removed entirely, not rendered in any form. `spec/capabilities/stock-analytics-dashboard.md`'s "Known Gaps / Future Work" tracks what each would need before it could be a real, un-stubbed feature.
...
- **Error/retry:** `GET /stocks/{symbol}/score` fails (including `404` — symbol not found) — human copy naming what failed ("Couldn't load {symbol}'s score. Please try again." or, for a 404 specifically, "We don't have data for {symbol}.") plus a tap-to-retry action, scoped to the score-hero/pillar area only (header and timeframe control still render — an error on the score fetch doesn't blank the whole screen). Never a raw error body or stack trace.
- **Populated:** score hero + pillar breakdown render real data. Nothing else renders below the pillar breakdown — this is the full screen this pass, not an in-between state waiting to resolve into something else.
...
- [ ] No "coming soon" text, disabled control, placeholder chart, or stub disclosure row renders anywhere on this screen — the price/chart, signal-drivers, and more-on-this-stock sections are entirely absent, not present in stub form; the screen goes directly from the score hero to "Why this score?" with nothing below it.
