...
| 4a | NSE Filings RAG | Built, scrape/parse/chunk/store only — `services/nse_scraper.py`, `CorporateFiling` table. **Known Gap (corrected 2026-08-30):** semantic embed/retrieve was never actually wired up despite earlier notes here claiming otherwise; the unused `all-MiniLM-L6-v2` model load was removed after it caused an OOM kill on the batch cron (~300MB, unconditional, on every run, for a value nothing read). Retrieval (`tool_nse_filings_rag`) is recency-only — see `spec/capabilities/nse-filings-rag.md` | consumed only via the chatbot (see row 4) | consumed only via the chatbot (see row 4) |
...
### Next — Mobile Tab Swipe Gestures (spec'd 2026-08-26, not yet built)
Left/right swipe-to-change-tab between the 5 visible tabs (Home, Portfolio, Health, Ask AI, More), additive to the existing tap-to-switch behavior — pure interaction/motion, no data-layer changes. Full design (chosen approach, feasibility findings, the 5-screen swipe order, what's excluded and why, gesture-conflict review, behavior-parity requirements, and the motion spec) is in `spec/ui.md` → "Tab Navigation — Swipe Gestures"; new mobile dependencies are noted in `spec/architecture.md` → `## Stack`. **Investigated, not a blocker:** `apps/mobile` doesn't currently have `react-native-tab-view`, `react-native-pager-view`, or `react-native-gesture-handler` installed — all three need adding via `npx expo install`. Expo Router 57 ships this navigator combination as a first-party, documented primitive (`TopTabs`, `expo-router/js-top-tabs`), which is itself evidence it's compatible with Expo SDK 57's New-Architecture-only environment. **Real build task once picked up:** replace `apps/mobile/app/(tabs)/_layout.tsx`'s `Tabs` with `TopTabs` + a fully custom bottom-tab-bar render component matching the current visual design exactly, move Alerts/Impulse Analyzer/Market News out of the swipeable navigator's route list entirely (not just hidden via `href: null`, which is insufficient once the navigator is swipe-capable), and replicate `tabBarHideOnKeyboard`/`sceneContainerStyle` parity manually since material-top-tabs has no built-in equivalents.
