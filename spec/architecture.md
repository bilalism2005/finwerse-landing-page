...
| IndianAPI, EODHD | Fundamentals, prices, news (batch only) | Same per-stock degradation pattern, plus a run-level circuit breaker: after `CIRCUIT_BREAKER_THRESHOLD` (5) consecutive per-stock failures against the same API, `BatchProcessor` stops calling it for the rest of that run instead of paying the retry/timeout cost on every remaining stock (added 2026-08-28 after a live EODHD news-endpoint outage caused the cron to blow past Render's 12h hard kill — see `harness/rules/` incident notes). `BatchProcessor.run()` also self-stops at a 10h30m time budget (`RUN_TIME_BUDGET`), logging a clear early-exit instead of being hard-killed; any stocks left unprocessed keep their old `computed_at` and are naturally first in line next run via the existing `skip_computed_today` check. |
