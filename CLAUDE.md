...
| `/design-review [screen or scope]` | HIG-grounded UI/UX audit (accessibility, contrast, platform conventions, motion) — vendored from `github.com/dickwu/apple-design-skill`, generalized for `apps/mobile`'s React Native (and `apps/web`, once wired to a real backend). Complements `harness/patterns/ui-ux.md`, which wins on any disagreement. |

**Auto-invocation is the default.** Analyze every prompt for whether it calls for one of these — a described feature/change → `/zero-shot-build`; a reported bug/error/failing test → `/zero-shot-fix`; a request for an audit, drift check, or "does spec match code" (or your own uncertainty after a change) → `/zero-shot-sync`; a request for a design/UI audit or accessibility check, **or any UI/visual design change** to `apps/mobile`/`apps/web` → also invoke `/design-review` (scoped to only the reference docs relevant to what actually changed — a pure logic/bug fix with no visual change doesn't need it). Invoke the matching skill(s) yourself; the user does not need to type the slash command. Each skill still only acts on the user's own words for what the idea/problem actually is — you supply `$ARGUMENTS` from what they said, you don't invent the idea or the diagnosis yourself. All four also work as an explicit `/slash-command` when the user does type one.
...
Not every change needs the multi-agent orchestration machinery — a small, well-scoped, single-surface fix can be done directly instead of spinning up separate `spec-writer`/`code-generator`/`qa-auditor` dispatches. **"Direct" scales down the orchestration, never the rigor.** Every change, regardless of size, still means: read the relevant `spec/` file(s) and `harness/rules/ai-agents.md` first, make the change, **actually run the real test/smoke command for that surface and show the output** (per rule 2 — "never claim a test passed if you didn't run it" has no size exception), update `spec/` if behavior changed, commit and push to `staging`. Reach for `/zero-shot-fix`/`/zero-shot-build`/`agent-builder` when the task is big enough, ambiguous enough, or multi-surface enough that the orchestration overhead itself earns its keep — not as a signal for when verification suddenly becomes required.
