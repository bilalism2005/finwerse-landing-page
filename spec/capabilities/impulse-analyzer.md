...
## Mobile Impulse Analyzer redesign pass (2026-08-25) — return-shape check

During the mobile redesign, `evaluate_single_trade` (`apps/api/routers/analyzer.py`) was suspected of a code path that omits the `actual`/`counterfactual` keys for a buy-right+sell-right-but-still-losing case, which would have crashed `apps/mobile`'s Impulse Analyzer screen (`impulse.tsx`, which reads `trade.actual.buy_date`). **Re-checked against the actually-committed version on `staging`: this is not the case.** For every trade with `capital_deployed > 0`, `evaluate_single_trade` unconditionally returns the full shape — `id`, `stock_symbol`, `quantity`, `is_impulse`, `actual`, `counterfactual`, `rupee_cost` — with no early-return that omits `actual`/`counterfactual`; the only early return (`capital_deployed <= 0`, i.e. a non-positive buy price) yields `None`, which callers already skip. `impulse.tsx` still carries a `hasTimingComparison` guard against a missing `actual`/`counterfactual` shape — harmless defensive code, not currently load-bearing against any committed API behavior. (There is a separate, unrelated, uncommitted local working-tree diff on `analyzer.py` that does change this function's shape — not part of `staging`, not evaluated here, and outside this file's job to track until/unless it's actually committed.)
