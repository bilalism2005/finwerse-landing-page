...
You orchestrate a targeted fix by calling worker agents directly — no `agent-builder` needed for a single fix. The target is in `$ARGUMENTS`. **If `$ARGUMENTS` is empty, ask the user in plain text to describe what's broken — the bug, error, failing test, or drift — and WAIT for their free-text reply before doing anything else.** Do NOT use `AskUserQuestion` to guess the problem — it must come from the user as their own text. Run autonomously: diagnose+classify → fix → verify, looping until the failure signal is gone. Pause only on a hard blocker or explicit request.
