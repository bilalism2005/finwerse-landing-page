...
| IndianAPI | Fundamentals (7-day cache) + daily per-stock news (`recentNews`, checked daily regardless of the fundamentals cache — batch only) | Per-stock degradation (`data_status = RATE_LIMITED/FAILED`), plus a run-level circuit breaker: after `CIRCUIT_BREAKER_THRESHOLD` (5) consecutive per-stock failures, `BatchProcessor` stops calling it for the rest of that run instead of paying the retry/timeout cost on every remaining stock (added 2026-08-28 after a live outage caused the cron to blow past Render's 12h hard kill). `BatchProcessor.run()` also self-stops at a 10h30m time budget (`RUN_TIME_BUDGET`), logging a clear early-exit instead of being hard-killed; any stocks left unprocessed keep their old `computed_at` and are naturally first in line next run via the existing `skip_computed_today` check. EODHD was dropped entirely for news (2026-08-31) — it has zero India/NSE exchange coverage, and its ticker-guessing workaround was silently returning wrong-company articles for most stocks. |
| FinVADER (VADER + SentiBignomics + Henry's earnings-press-release lexicons — Apache 2.0/CC BY, no commercial-license gate) | Free, in-process, per-article tone scoring for every IndianAPI news article (batch only) | Runs locally, no network dependency, no failure mode of its own. Escalates to a Groq call only when its own signal can't be trusted — combined score too close to zero, or its two component lexicons meaningfully disagree (empirically real failure patterns: SentiBignomics alone correctly flags "rising debt/cost" as negative while Henry's alone inverts it; SentiBignomics dilutes severe events like fraud that Henry's alone scores correctly). **Escalation rate is higher than first assumed** — broader testing post-launch (2026-08-31) found routine positive earnings language (e.g. "beat estimates", "record dividend") often triggers the disagreement check too, since SentiBignomics has weak coverage of that vocabulary versus Henry's; roughly half of realistic headlines escalated in a 6-article sample, not "a small fraction." Groq's fallback score was correct in every case tested, so this doesn't affect correctness — but the `FINVADER_DISAGREEMENT_THRESHOLD = 0.4` cost assumption needs revisiting with real production volume before treating the "~few dollars/month" estimate as settled. |
| Groq | Chatbot (Node 1 routing + Node 2 synthesis) + Bottleneck Report + escalated news-sentiment scoring (direction + estimated impact magnitude, -100..100) | 3-model failover chain per call site for the chatbot/Bottleneck Report; the sentiment-scoring call has its own `CIRCUIT_BREAKER_THRESHOLD`-based breaker (`_groq_sentiment_circuit_open`) — falls back to FinVADER's own (less reliable but still directionally-useful) score when open, rather than blocking scoring entirely |
...
- **Mobile tab-swipe gestures (spec'd 2026-08-26, not yet built — `spec/ui.md` → "Tab Navigation — Swipe Gestures"):** `expo-router`'s built-in `TopTabs` (from the `expo-router/js-top-tabs` entry point — `withLayoutContext(createMaterialTopTabNavigator().Navigator)`, vendored inside `expo-router` itself, no separate `@react-navigation/material-top-tabs` install needed), restyled with a fully custom bottom-tab-bar renderer. Requires adding `react-native-tab-view` (the actual pager/gesture runtime `TopTabs` calls at runtime) plus its peers `react-native-pager-view` and `react-native-gesture-handler` — none of the three are in `apps/mobile/package.json` today. Install via `npx expo install <pkg>` to get the SDK-57-blessed versions, not bare `npm install`.
