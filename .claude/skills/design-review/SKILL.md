...
description: >
  ...
  vendored from github.com/dickwu/apple-design-skill for finwerse's apps/mobile (Expo/React Native)
  and, once wired to a real backend, apps/web. Use this skill to audit, review, critique, or improve
  any UI/UX design. Triggers when the user asks for a design audit, UI review, HIG/accessibility
  compliance check, design feedback, or improvement on any screen — or describes/requests a UI or
  visual design change to apps/mobile or apps/web. Also use when the user uploads screenshots,
  mockups, wireframes, or design specs and wants feedback. Even a bare "review my design" or "is
  this good UI" should trigger this skill.
---

**Auto-invocation policy:** invoke this skill on your own analysis whenever the user asks for a
design/UI audit, accessibility check, or feedback on a screen's design — they do not need to type
`/design-review` explicitly. Also invoke it proactively as part of any `/zero-shot-build` or direct
`code-generator` dispatch that changes a screen's visual design on `apps/mobile` or `apps/web`,
loading only the reference files relevant to what actually changed (per Step 2 below) rather than
the full 8-file default set — a pure logic/bug fix with no visual change does not need this skill.

This vendored copy tracks `references/hig-lookup.md` + `references/hig/*.md` verbatim from the
source repo (github.com/dickwu/apple-design-skill) — update by re-pulling that repo, don't
hand-edit the reference docs. Finwerse's own `harness/patterns/ui-ux.md` is the project-specific
UI/UX rulebook and takes precedence wherever the two disagree — this skill supplies the general,
platform-grounded checks (contrast, touch targets, platform conventions, motion) that
`harness/patterns/ui-ux.md` doesn't itself enumerate.
